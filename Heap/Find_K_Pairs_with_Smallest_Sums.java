/*
ou are given two integer arrays nums1 and nums2 sorted in ascending order and an integer k.

Define a pair (u,v) which consists of one element from the first array and one element from the second array.

Find the k pairs (u1,v1),(u2,v2) ...(uk,vk) with the smallest sums.

Example 1:

Input: nums1 = [1,7,11], nums2 = [2,4,6], k = 3
Output: [[1,2],[1,4],[1,6]] 
Explanation: The first 3 pairs are returned from the sequence: 
             [1,2],[1,4],[1,6],[7,2],[7,4],[11,2],[7,6],[11,4],[11,6]
Example 2:

Input: nums1 = [1,1,2], nums2 = [1,2,3], k = 2
Output: [1,1],[1,1]
Explanation: The first 2 pairs are returned from the sequence: 
             [1,1],[1,1],[1,2],[2,1],[1,2],[2,2],[1,3],[1,3],[2,3]
Example 3:

Input: nums1 = [1,2], nums2 = [3], k = 3
Output: [1,3],[2,3]
Explanation: All possible pairs are returned from the sequence: [1,3],[2,3]
*/

/*
Time: O(K^2 log K)
Space: O(K)
*/
class Solution {
    public List<List<Integer>> kSmallestPairs(int[] nums1, int[] nums2, int k) {
        PriorityQueue<List<Integer>> maxHeap = new PriorityQueue<>((n1,n2)->(n2.get(0)+n2.get(1)-n1.get(0)-n1.get(1)));
        for(int i=0;i<nums1.length && i<k;i++){
            for(int j=0;j<nums2.length && j<k;j++){
                // add all pairs until maxHeap.size() becomes K
                if(maxHeap.size()<k){
                    maxHeap.add(Arrays.asList(new Integer[]{nums1[i], nums2[j]}));
                } else {
                    // if current pair sum > maxHeap.peek(), break as it is of no use going forward in j's direction.
                    if(nums1[i]+nums2[j] > maxHeap.peek().get(0)+maxHeap.peek().get(1)){
                        break;
                    } else {
                        // remove top element
                        maxHeap.poll();
                        // push current pair to the heap.
                        maxHeap.add(Arrays.asList(new Integer[]{nums1[i], nums2[j]}));
                    }
                }
            }
        }
        List<List<Integer>> result = new ArrayList<>();
        result.addAll(maxHeap);
        return result;
    }
}